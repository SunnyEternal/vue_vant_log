  ...
  <div class="home-container">
    ...
    <div v-show="!finish">
      ...
      <div class="handle-main" v-show="!addBtnVisible">
        ...
        <van-search v-model="searchValue" @search="onSearch" placeholder="请输入关键字" />
        <van-list>
        <van-cell v-show="showSearch" v-for="(item, index) in dataList.slice(0, 10)" :key="index">
          {{ item }}
        </van-cell>
      </van-list>
...
export default {
  data() {
    return {
      ...
      searchValue: '',  // 搜索框内输入的值
      dataList: [],     // 模糊搜索到的数据
      // dataList: ["孕妇可以吃榴莲吗", "孕妇可以吃螃蟹吗", "孕妇孕反应什么样？", "孕妇睡不好怎么办？", "孕妇吃不下饭怎么办？", "孕妇可以化妆吗？", "孕妇可以游泳吗？", "孕妇失眠怎么办？", "孕妇如何防晒？", "孕妇如何控制体重？"],
      showSearch: false
    };
  },
  created() {
  ...
  methods: {
    ...
    onSearch() {
      ...
      // TODO: 实现模糊搜索逻辑
      setTimeout(function() {
        this.dataList = ["孕妇可以吃榴莲吗", "孕妇可以吃螃蟹吗", "孕妇孕反应什么样？", "孕妇睡不好怎么办？", "孕妇吃不下饭怎么办？", "孕妇可以化妆吗？", "孕妇可以游泳吗？", "孕妇失眠怎么办？", "孕妇如何防晒？", "孕妇如何控制体重？"]
        this.showSearch = true
      }, 800)
